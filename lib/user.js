...
User.prototype.login = function() {
	...
	if (!settings.quiet) {console.log('Logging in user %j', that.email);}
	...
	reqHelper.post(url, body).then(function(result) {
		that.import(result);
		if (!settings.quiet) {console.log('User %j has logged in successfully', that.email);}
...
User.prototype.logout = function() {
	...
	.then(function(result) {
		if (!settings.quiet) {console.log('User %j has logged out successfully', that.email);}
		return that;
	});
};
