 ...
 */
 ...
 .command('copy', {
	 ...
	 action: function(args, flags) {
		 ...
		 if (!flags.drive || !isNaN(parseFloat(flags.drive)) && 
				 isFinite(flags.drive) && flags.drive < 1) {
			 logHelper.consoleOutput('Error: --drive must be set and it must have a numeric value >= 1');
			 copy.usage();
			 return;
		 }
	 ...
	 shortcut: '-d',
	 description: 'drive folder where to copy transfer into',
	 required: 'folder idx'
 }).option('--interval', {
	 shortcut: '-i',
	 description: 'Run in loop, with delay between checks',
