...
Reservation.prototype.create = function() {
	...
	if (!settings.quiet) {console.log('Creating new reservation');}
	...
	return reqHelper.post(
		...
		if (!settings.quiet) {console.log('Reservation %s created', that.token);}
		return that;
...
Reservation.prototype.upload = function() {
	...
	var uploadFile = function(file) {
		...
		.then(function(result) {
			if (!settings.quiet) {console.log('Upload of %s completed', file.objectId);}
			return result;
		});
	};	
...
Reservation.prototype.confirm = function() {
	...
	if (!settings.quiet) {console.log('Confirming reservation %s', this.token);}
	return reqHelper.post(url, {})
	.then(function(result) {
		if (!settings.quiet) {console.log('Reservation confirmed');}
		return result;
	});
};
