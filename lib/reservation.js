...
Reservation.prototype.create = function() {
	...
	return reqHelper.post(
		...
		console.log('Reservation %s created', that.token);
		return that;
	}).catch(function(error) {
		console.log('Reservation not created');
		throw error;
	});
};

Reservation.showProgressBar = function(file) {
	
	var req = https.request({
		host: settings.getBaseUrl().split("//")[1],
		port: 443,
		path: file.path
	});
...
Reservation.prototype.upload = function() {
	...
	var uploadFile = function(file) {
		...
		console.log('Uploading %s (%s Bytes)', 
			file.objectId, 
			file.size.toLocaleString()
		);

		//Reservation.showProgressBar(file);
		//console.log('-----------reservation.js 290--------------------', file);
		
		return reqHelper.putFile(url, file.path, that.token)
		.then(function(result) {
			console.log('Upload of %s completed', file.objectId);
			
			/*
			//deleting the via node-zip-dir(zip program) created zip files
			for(var i=0; i<that.files.length; i++) {
				if(i === fileCounter && that.files[i].name.slice(-3) === "zip" && that.files[i].size <= -1) {
					fs.unlinkSync(that.files[i].path);
				}
			}
			fileCounter++;
			*/
...
Reservation.prototype.confirm = function() {
	...
	console.log('Confirming reservation %s', this.token);
