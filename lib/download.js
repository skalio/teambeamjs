...
exports.doit = function(options) {
	console.log('Downloading transfers for %j', process.principal.email);
	/*
	...
	.then(function(transfers) {
		...
		transfers.forEach(function(transfer) {
			...
			if (transfer.folderIdx) {console.log("download.js--------------1. if");
				if (settings.verbose) {
			...
			if ((transfer.downloadCounter > 0) && !options['include-old']) {console.log("download.js--------------2. if");
				if (settings.verbose) {
			...
			if ((transfer.receiver.email !== process.principal.email) && !options['include-sent']) {console.log("download.js--------------3. if");
				if (settings.verbose) {
			...
			}).then(function() {console.log("download.js--------------77", transferDir);
