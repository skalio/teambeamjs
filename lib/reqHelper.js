 ...
 * @returns {Q.Promise}
 */
exports.getAsStream = function(url, basedir, objectId) {console.log("12------------------#########################################################--------------------");
	/*g_options.method = 'get';
	g_options.stream = true;
	g_options.path = basedir;
	delete g_options.headers["Content-Length"];*/
	return exports.call(
			url,// g_options
			{
				method: 'get',
...
function _makeRequest(url, options) {
	...
	} else if(options.stream) {console.log('reqHelper.js--------------------------215: ', options);
		//fs.createReadStream(options.url).pipe(session(options, callback));
		session(options, callback).pipe(fs.createWriteStream(options.path + "/" + options.objectId));
		//session(options, callback).pipe(fs.createWriteStream('2256425-playing-cards-full-house-poker-hand-on-white-Stock-Photo.jpg'));
		//fs.createReadStream(options.url).pipe(session(options, callback));
		//session(options, callback).pipe(fs.createWriteStream(options.path));
		//session(options, callback).pipe(fs.createWriteStream(options.path));
				/*.on('close', function() {
					callback();
				});*/
	//	session(options, callback);
		/*session(options, function (err, res, body) {console.log('reqHelper.js--------------------------215: ', options);
			if (err)
				throw err;
