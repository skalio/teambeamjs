...
File.prototype.download = function(basedir, options) {
	...
	return reqHelper.getAsStream(that.url, basedir, that.objectId)
	.then(function(response) {console.log('file.js--------------------------105: ', response.statusCode);
		if (response.statusCode >= 400) {
			deferred.reject(new Error('Request failed: '+ response.body.error.message));
		}
		/*lastProgressTimestamp = 0;
		response.on('progress', function(completedBytes, totalBytes) {console.log("14------------------------------");
			if (!settings.verbose) {
				return;
			}
		...
		console.log("15------------------------------");
		response.on('end', function() {console.log("16------------------------------");
			console.log('Download completed for %s', that.objectId);
			deferred.resolve();
		});

		response.on('error', function(err) {console.log("17------------------------------");
			// if (bar) bar.cancel();
			deferred.reject(err);
		});
		console.log("18------------------------------");
