...
Transfer.getAll = function() {
	...
	.then(function(result) {
		...
		if (!settings.quiet) {console.log('Response contains %j entries', result.total);}
		var transfers = [],
			data;
...
Transfer.prototype.downloadFiles = function(basedir, options) {
	...
	options = options || {};
	if (!settings.quiet) {console.log('Downloading files for transfer %j', that.recipientId);}
	...
	return promise.then(function() {
		if (!settings.quiet) {console.log('All files for transfer %j have been downloaded', that.recipientId);}
		return Q.resolve();
	});
