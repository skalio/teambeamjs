...
var Transfer = function(input) {
	...
	this.accessTimestamp = moment(input.accessTimestamp || 0).toDate();
	...
	this.isUnread = input.isUnread || false;
...
Transfer.prototype.export = function() {
	var i,
		obj = {},
		prop,
		fields = ['recipientId', 'sender', 'receiver', 'uploadTimestamp', 'accessTimestamp',
		'downloadTimestamp', 'expirationTimestamp',
		'subject', 'description', 'folderIdx', 'priority', 'isBounced', 'isUnread',
...
/**
 * Copies the transfer object to the given folder
 * @param {string} folderIdx
 * @returns {Transfer}
 */
 Transfer.prototype.copy = function(folderIdx) {
	var that = this;
	var url = settings.getBaseUrl() +'/rs/v1/transfers/'+ that.recipientId + '/copy';
