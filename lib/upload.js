	...
	if (options.files) {
		options.files.forEach(function(filename) {
			if (fs.existsSync(filename)) {				
				...
				if(stat.isDirectory()) {
					var zipFilename = filename.split(path.sep).pop() + ".zip";
					if (fs.existsSync(tmpFolder.name + path.sep + zipFilename)) {
						//rename the zip-file if this exists in tmpFolder
						zipFilename = filename.split(path.sep).pop() + "(" + copyDirCounter + ").zip";
						copyDirCounter++;
					}
					var thisFile = tmpFolder.name + path.sep + zipFilename;
					zip.zip(filename, thisFile);
					reservation.addFile(tmpFolder.name + path.sep + zipFilename);
				} else {
