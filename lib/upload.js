...
exports.doit = function(options, loggedinUser) {
	...
	if (existFile) {
		...
		// ttl
		if (options.ttl) {
			var ttlArr = loggedinUser.limitations.expirationInterval;
			if(ttlArr.indexOf(options.ttl) === -1) {	//the given ttl doesn't exist and we set the default ttl
				readyToUpload = false;
				...
				request(environment, function (error, response, body) {
					if (!error && response.statusCode === 200) {
						...
						options.ttl = defaultVal.expiration.default;
						reservation.setTtl(options.ttl);
						console.log("Your given ttl doesn't exist. The possible ttl are %j and the default ttl %j is automatically selected", ttlArr, options.ttl);
						readyToUpload = true;
						...
						if( fs.existsSync(path) ) {
							fs.readdirSync(path).forEach(function (file, index) {
								...
								if (fs.lstatSync(curPath).isDirectory()) { 
									...
									//delete files to get a empty folder
